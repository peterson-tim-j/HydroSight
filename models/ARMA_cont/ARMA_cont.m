classdef ARMA_cont < model_abstract
    %ARMA_CONT Summary of this class goes here
    ...
    methods
        function obj = ARMA_cont(bore_ID, obsHead, forcingData_data,  forcingData_colnames, siteCoordinates, varargin)
            ...
            % Set option for using GPU
            if varargin{1} == true || varargin{1} == false
                obj.variables.useGPU = varargin{1};
            else
                error('The model ARMA_cont only accepts one option defining if GPU are to be used. It must be true or false');
            end
        ...
        function [head, colnames, noise] = solve(obj, time_points, tor_min, tor_max)
            ...
            [~, head] = objectiveFunction(params, time_points_all, obj);            
        ...
        function [params_initial, time_points] = calibration_initialise(obj, t_start, t_end)
            ...
            % Undertake linear regression of first 5 time points to
            % estimate the initial slope and intercept.
            t = obj.inputData.head(t_filt,1);
            head = obj.inputData.head(t_filt,2);
            startDate = min(t);
            maxTrendEstYears = 5;
            t_filt_polyfit = t<=ceil(startDate+maxTrendEstYears*365);
            if sum(t_filt_polyfit)<12
                t_filt_polyfit = [1:min(length(t),12)]';
            end
            if sum(t_filt_polyfit)>2                
                params = polyfit((t(t_filt_polyfit)-startDate)./365,head(t_filt_polyfit)-mean(obj.inputData.head(t_filt,2)),1);
                obj.variables.initialTrend = params(1);
                obj.variables.initialHead = params(2);
                
            else
               error('At least 2 head observations are required.');
            end  
            ...
            || exp(mean(log( 1- exp( -2.*beta_upperLimit .* delta_time) ))) < eps()
                beta_upperLimit = beta_upperLimit - 0.01;
                if beta_upperLimit <= eps()                                   
                    break;
                end
            end
            if beta_upperLimit <= eps()
                obj.variables.beta_upperLimit = inf;
            else
            ...
            if ~isinf(obj.variables.beta_upperLimit)
                ...
                > obsHead_std                       
                    ...
                    beta_lowerLimit = beta_lowerLimit + 0.01;
                end
        ...
        function [objFn, h_star, h_forecast] = objectiveFunction(params, time_points, obj)                
            ...
            % Calculate the auto-regressive componant of the model using
            % the following MEX function.            
%             [h_ar, h_forecast] = doExpSmoothing(  time_points, ...
%                                                         obj.inputData.head(:,2), ...
%                                                         obj.variables.isObsTimePoints, ...
%                                                         obj.variables.meanHead_calib, ...
%                                                         obj.variables.meanDelta_t, ...
%                                                         alpha, ...
%                                                         gamma, ...
%                                                         zeta, ...
%                                                         obj.variables.initialTrend_calib, ...
%                                                         obj.variables.initialHead_calib);
