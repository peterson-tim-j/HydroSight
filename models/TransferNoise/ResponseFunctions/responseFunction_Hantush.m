        ...
        function result = theta(obj, t)           
            ...
            for i=1: size(obj.settings.pumpingBores,1)
                ...
                if isfield(obj.settings.pumpingBores{i,1},'imageBoreID')
                    ...
                    result(:,i) = bsxfun(@plus, - 10.^obj.alpha./t.*exp( -10.^obj.beta * (pumpDistances.^2 ./t) - 10.^obj.gamma .* t ), ...
                        sum(bsxfun(@times, imageWellMultiplier' , bsxfun(@times, 10^obj.alpha./t , exp( bsxfun(@plus, -10^obj.beta * bsxfun(@rdivide,imageDistances'.^2,t),-10.^obj.gamma.*t)))),2));
                else                    
                    result(:,i) = - 10.^obj.alpha./t.*exp( -10.^obj.beta * (pumpDistances.^2 ./t) - 10.^obj.gamma .* t );
                end
            end  
