...
%   defined, calibrated and solved using GroundwaterStatisticsToolbox.m.
%
...
%       function. 
%
%       - Pumping and streamflow influence on heads can be included using
%       responseFunction_Hantush and responseFunction_Bruggeman impluse
%       respnse functions respectively.
%
...
%       observed and modelled is head is derived and the innovation is
...
%  Constructor for linear and nonlinear Transfer Function Noise model
%
% Syntax:
...
%
% Description:
%   Builds the model_TFN object, delares initial parameters and sets the
%   observation and forcing data withint the object. This is the method
%   should be called from GroundwaterStatisticsToolbox.
%
...
%       for example, the parameterised Pearson's weightign function for 
...
%       The third column gives the type of timage well. The availabes
...
%
% Description:
%   Finalised the model following calibration and assigns the final 
%   parameters and additional variables to the object for latter simulation. 
...
%   input componanat and parameter names. This method is predominately used
...
% Returns the physical limits to each model parameter.
%
% Syntax:
                    ...
                    for j=1: size(companant_params,1)
                       if ~strcmp( companant_params(j), 'type')
                            if strcmp(currentField,'et') && strcmp( companant_params(j), 'k')
                                params_upperLimit = [params_upperLimit; inf];
                           ...
                           || strcmp(currentField,'landchange') && strcmp( companant_params(j), 'et_scalar'))
                               ...
                               % functions.
                               params_upperLimit = [params_upperLimit; inf];
                               params_lowerLimit = [params_lowerLimit; -inf];                                
                            else
                                % This parameter is assumed to be the noise parameter 'alpha'.                                                        
                                params_upperLimit = [params_upperLimit; inf];                                                        
                                params_lowerLimit = [params_lowerLimit; log10(sqrt(eps()))];                                
                            end                           
                       end
                    end
                end
...
% Returns the plausible limits to each model parameter.
%
% Syntax:
...
% Returns logical vector for validity or each parameter.
%
% Syntax:
%   [params_upperLimit, params_lowerLimit] = getParameters_plausibleLimit(obj, params, time_points)
%
% Description:
%   Cycles though all model componants and parameters and returns a logical 
...
%   26 Sept 2014   
        ...
        for ii=1: size(companants,1)
            ...
            % Check the alphanoise parameter is large enogh not to cause numerical
...
% Private method calculating the contribution to the head from each model componant.
%
% Syntax:
%   [h_star, colnames] = get_h_star(obj, time_points, tor_min, tor_max)
%
...
%   private and is called by either solve() or objectiveFunction() public
            ...
            for i=1:nCompanants
                ...
                for j=1: size(theta_est_temp,2)
                    ...
                    if ischar(obj.variables.(companants{i}).forcingData_colnames)
                        colnames{iOutputColumns} = obj.variables.(companants{i}).forcingData_colnames;
                    else
                        colnames{iOutputColumns} = obj.variables.(companants{i}).forcingData_colnames{j};
                    end
                end
            end
