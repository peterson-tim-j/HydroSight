...
%    7. Numerous edits were made to support matlabe 2015b+. See history
%       below. Note, the functions for the buttons has not been tested.
...
%    1.3 2007-05-07: fix for Matlab 7.0.4 as per Sebastian H�lz; fix default headers per Selig Sechzer; full P-V parameters processing
...
% $Revision: 1.6 $  $Date: 2015/01/07 21:52:53 $
  ...
  %try
      ...
      if isa(hContainer,'figure') || isa(hContainer,'matlab.ui.Figure')
          pnContainerPos = getpixelposition(paramsStruct.container,0);  % Fix for Matlab 7.0.4 as per Sebastian H�lz
          pnContainerPos(1:2) = 0;
      else
          pnContainerPos = getpixelposition(paramsStruct.container,1);  % Fix for Matlab 7.0.4 as per Sebastian H�lz
      end
      ...
      if isempty(paramsStruct.data) && max(size(paramsStruct.data))==0
          numRows = 0;
      ...
      % uitable command edited by T Peterson for matlab
      % 2015b support.
      try
          mtable = uitable('Parent', hContainer, 'position',tablePosition, 'Data',paramsStruct.data, 'ColumnName',paramsStruct.headers, paramsStruct.extra{:});
      catch
          mtable = uitable(hContainer, 'position',tablePosition, 'Data',paramsStruct.data, 'ColumnNames',paramsStruct.headers);
      end
      ...
      % Enable multiple row selection, auto-column resize, and
      % auto-scrollbars. First 3 lines commented by T Peterson for matlab
      % 2015b support.
      ...
      %jtable.setScrollableTracksViewportHeight(true);
      %jtable.setScrollableTracksViewportWidth(true);
      ...
      % Process optional args. TJP edit 2016.
...
% handles    structure with handles and user data (see GUIDATA)
  %try
      ...
      % Delete all table rows. TJP Edit 2016.
      mtable.table.Data=[];
