...
if (USE_CUSTOM_GANYMED_LIB > 0)
    ...
    % FROM CLEONDRIS
    ganymed_java_library = 'ganymed-ssh2-build210'; 
    ganymed_java_library_zip = [ganymed_java_library '.zip'];
    
    ganymed_java_library_http = sprintf('http://www.ganymed.ethz.ch/ssh2/%s',ganymed_java_library_zip);
    %ganymed_java_library_http = sprintf('http://www.cleondrisch/ssh2/%s',ganymed_java_library_zip);
        ...
        if (exist(ganymed_java_library_jar_path,'file'))
            ...
            if (USE_CUSTOM_GANYMED_LIB == 0) %try to download official (non-custom) version 
                if (exist(ganymed_java_library_zip))
                    ...
                    [fname, status] = urlwrite(ganymed_java_library_http, ganymed_java_library_zip);
                    % Tim Peterson 2016
                    if status==0
                        error(['Error downloading ',ganymed_java_library_http]);
                        return;
                    end
                end
