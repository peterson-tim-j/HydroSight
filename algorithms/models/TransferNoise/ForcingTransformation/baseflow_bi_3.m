classdef baseflow_bi_3 < forcingTransform_abstract
    ...
    % storage threshold is exceeded, with same asymptotes for
    % gaining/losing
...
% user of the available model types. 
    methods(Static)
        ...
        function modelDescription = modelDescription()
           modelDescription = {'baseflow_bi_3', ...
                               '', ...
    ...
    %% Constructor of the baseflow_bi_3 class
    
    methods
        function obj = baseflow_bi_3(bore_ID, forcingData_data,  forcingData_colnames, siteCoordinates, forcingData_reqCols, modelOptions)
...
%                 head_col = obj.settings.forcingData_colnames(filt,:);
            ...
            obj.variables.baseFlow = obj.linear_scaler .* (1 - exp( - obj.exponential_scaler .* (obj.variables.head - obj.head_threshold))) ;
            ...
            % func =  K2 [1 - exp(- K3· Delta_H)] - This relationship permits a rapid increase in the flow for small head changes when the head difference is small, but postulates maximum flows that cannot be exceeded as long as the head differ- ence becomes larger
