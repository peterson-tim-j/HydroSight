classdef baseflow_bi_2 < forcingTransform_abstract
    % Defines the behaviour of baseflow according to the GW head and a scaled weighted rate. 
    
    % Detailed explanation goes here
    % Description:  Linear outflow - inflow from a reservoir if a storage
    % threshold is exceeded, but with different slopes for gaining/losing
        
    properties (GetAccess=public, SetAccess=protected)
        
        % Model Parameters
        %----------------------------------------------------------------
        linear_gaining_scaler   % - time coefficient [d-1]
        linear_losing_scaler   % - time coefficient [d-1]
...
% user of the available model types. 
    methods(Static)
        ...
        function modelDescription = modelDescription()
           modelDescription = {'baseflow_bi_2', ...
                               '', ...
    ...
    %% Constructor of the baseflow_bi_2 class
    
    methods
        function obj = baseflow_bi_2(bore_ID, forcingData_data,  forcingData_colnames, siteCoordinates, forcingData_reqCols, modelOptions)
...
%             obj = obj@baseflow(bore_ID, forcingData_data,  forcingData_colnames, siteCoordinates, forcingData_reqCols, modelOptions);
            ...
            obj.linear_gaining_scaler = 0; % initial guess for - time coefficient [d-1]
            obj.linear_losing_scaler = 0; % initial guess for - time coefficient [d-1]
        ...
        function [params, param_names] = getParameters(obj)            
           params = [ obj.linear_gaining_scaler; obj.linear_losing_scaler;  obj.head_threshold];
           param_names = {'linear_gaining_scaler';'linear_losing_scaler'; 'head_threshold'};
        end
...
%                 head_col = obj.settings.forcingData_colnames(filt,:);
            ...
            % Constraints:  -
            % @(Inputs):    p1   - linear time coefficient [day-1]
            %               p2   - storage threshold for flow generation [mAHD]
            %               S    - current storage [mAHD]
            % func = p1 .* (S-p2);
